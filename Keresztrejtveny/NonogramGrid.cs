...
namespace Nonogram
{
    public class NonogramGrid
    {
        ...
        public void GenerateEasyNonogram(int gridLeft, int gridTop)
        {
            if (form.isColor)
            {
                form.easyColors = GetFourRandomColors(); // Itt frissítjük a globális tömböt
            }
            ...
            using (Graphics g = Graphics.FromImage(bmp))
            {
                ...
                for (int i = 0; i < rows; i++)
                {
                    for (int j = 0; j < cols; j++)
                    {
                        ...
                        if (form.chkXMode.Checked && form.userXMark[i, j])
                        {
                            // fehér háttér
                            g.FillRectangle(Brushes.White, cellRect);

                            // X rajzolása szürkével, arányosan
                            float fontSize = Math.Min(cellWidth, cellHeight) * 0.7f; // arányos betűméret
        ...
        public void GenerateHardNonogram()
        {
            if (form.isColor)
            {
                form.hardColors = GetFourRandomColors(); // Új színek a nehéz pályához
            }
            int gridLeft = 20;
            int gridTop = Math.Max(500, form.chkShowX.Bottom) + 20;

            int w = 20, h = 20;
            int targetPixels = 200; // Pontosan 200 pixel
            Random rnd = form.rnd;
            bool isBoardGood = false;
