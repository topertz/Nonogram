        ...
        public void GenerateNonogram(int gridLeft, int gridTop, int width, int height, int targetPixels, int maxAttempts)
        {
            ...
            while (!isBoardGood && attemptOverall < maxAttempts)
            {
                ...
                while (currentPixels < targetPixels && attempts < maxAttempts)
                {
                    ...
                    int c = form.rnd.Next(0, width - len + 1);

                    // szabad-e a hely
                    bool free = true;
                    for (int k = 0; k < len; k++)
                        if (finalBW[r, c + k] == 1) { free = false; break; }
                    if (!free) continue;
                    ...
                    {
                        // Nézzük a bal oldalt
                        if (c > 0 && finalBW[r, c - 1] == 1) len = 2;
                        // Nézzük a jobb oldalt
                        else if (c + 1 < width && finalBW[r, c + 1] == 1) { c = c - 1; len = 2; }
                    ...
                    // Blokk elhelyezése
                    for (int i = 0; i < len && currentPixels < targetPixels; i++)
                    {
                        finalBW[r, c + i] = 1;
                        finalColors[r, c + i] = blockColor;
                        currentPixels++;
                    }
                }
        ...
        public void CreateGridUI(int gridLeft, int gridTop)
        {
            ...
            for (int i = 0; i < row; i++)
            {
                for (int j = 0; j < col; j++)
                {
                    ...
                    btn.Paint += (s, e) =>
                    {
                        ...
                        if (btn == null) return;
                        Point point = (Point)btn.Tag;
                        int rowIndex = point.X, colIndex = point.Y;
                        if(render.showHints)
                        {
                        ...
                        {
                            if (rowIndex % 5 == 0) e.Graphics.DrawLine(thickPen, 0, 0, btn.Width, 0);
                            if (colIndex % 5 == 0) e.Graphics.DrawLine(thickPen, 0, 0, 0, btn.Height);
                            if (colIndex == col - 1) e.Graphics.DrawLine(thickPen, btn.Width - 1, 0, btn.Width - 1, btn.Height);
                            if (rowIndex == row - 1) e.Graphics.DrawLine(thickPen, 0, btn.Height - 1, btn.Width, btn.Height - 1);
                        }
                    };
                    btn.MouseDown += (s, e) =>
                    {
            ...
            if (isColor)
            {
                ...
                for (int i = 0; i < row; i++)
                    for (int j = 0; j < col; j++)
                    {
                        Color color = solutionColorRGB[i, j];
        ...
        public void GenerateNonogramFromImage(Image img, int gridLeft, int gridTop)
        {
            ...
            for (int i = 0; i < row; i++)
            {
                ...
                while (j < col)
                {
                    ...
                    {
                        Color color = solutionColorRGB[i, k];
            ...
            for (int j = 0; j < col; j++)
            {
                ...
                while (i < row)
                {
                    ...
                    {
                        Color color = solutionColorRGB[k, j];
                        if (isColor && count > 0 && !render.AreColorsSimilar(seed, color, colorSimilarityThreshold)) break;
                        count++; k++;
                    }
                    clues.Add(count);
