...
namespace Grafilogika_alkalmazas_keszitese
{
    public partial class NonogramSmartAI
    {
            ...
            for (int r = 0; r < grid.row; r++)
            {
                for (int c = 0; c < grid.col; c++)
                {
                    ...
                    if (!IsFilled(r, c) && !IsX(r, c))
                    {
                        ...
                        HashSet<int> allowedInRow = grid.rowClueColors[r].Select(clr => clr.ToArgb()).ToHashSet();
                        HashSet<int> allowedInCol = grid.colClueColors[c].Select(clr => clr.ToArgb()).ToHashSet();
                        ...
                        List<(bool isRow, int lineIdx, int blockIdx)> candidates = GetCandidateBlocks(r, c).ToList();
                        ...
                        foreach ((bool isRow, int lineIdx, int blockIdx) in candidates)
                        {
                            ...
                            for (int start = leftmost[blockIdx]; start <= rightmost[blockIdx]; start++)
                            {
                                ...
                                for (int i = 0; i < blockLen; i++)
                                {
                                    ...
                                    await SetCell(currR, currC, blockColor, "Spekulatív elhelyezés (backtracking)");
                                }
        ...
        private IEnumerable<(bool isRow, int lineIdx, int blockIdx)> GetCandidateBlocks(int r, int c)
        {
            ...
            HashSet<int> allowedColorsInRow = grid.rowClueColors[r].Select(clr => clr.ToArgb()).ToHashSet();
            HashSet<int> allowedColorsInCol = grid.colClueColors[c].Select(clr => clr.ToArgb()).ToHashSet();
        ...
        private async Task<bool> AnalyzeLine(int index, bool isRow)
        {
            ...
            for (int p = 0; p < length; p++)
            {
                ...
                HashSet<int> rowColorSet = grid.rowClues[r].Select((_, idx) => GetClueColor(r, true, idx).ToArgb()).ToHashSet();
                HashSet<int> colColorSet = grid.colClues[c].Select((_, idx) => GetClueColor(c, false, idx).ToArgb()).ToHashSet();

                rowColorSet.IntersectWith(colColorSet);
                if (rowColorSet.Count == 0)
                {
                    if (await SetX(r, c, "Szín-összeférhetetlenség (sor vagy oszlop metszet üres)")) return true;
                }
            }
            ...
            for (int p = 0; p < length; p++)
            {
                ...
                if (!canAnyBlockReach)
                {
                    if (await SetX(r, c, "Egyik blokk sem érhet el ide")) return true;
                }
            }
                    ...
                    if (count == 1)
                    {
                        ...
                        for (int fillP = overlapStart; fillP <= overlapEnd; fillP++)
                        {
                            ...
                            if (!IsFilled(fr, fc) && !IsX(fr, fc))
                            {
                                ...
                                int[] crossClues = isRow ? grid.colClues[fc] : grid.rowClues[fr];
