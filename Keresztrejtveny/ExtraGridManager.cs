    ...
    public class ExtraGridManager
    {
        ...
        public bool PrepareTextBoxColors()
        {
            try
            {
                ...
                // --- Debug kiíratás ---
                Console.WriteLine("=== TextBox Colors Debug ===");

                foreach (var kvp in form.textBoxColors)
                {
                    RichTextBox rtb = kvp.Key;
                    List<Color> colors = kvp.Value;

                    string colorNames = colors.Count == 0
                        ? "(üres)"
                        : string.Join(", ", colors.Select(c => c == Color.Empty ? "Empty" : c.Name));

                    Console.WriteLine($"ClueTextBox {rtb.Name}: {colorNames}");
                }
            ...
            for (int i = 0; i < row; i++)
            {
                for (int j = 0; j < col; j++)
                {
                    ...
                    else
                    {
                        ...
                        form.gridButtons[i, j].Font = new Font("Arial", cellSize / 2, FontStyle.Bold);
                    }
                }
            }
        }
        ...
        public bool SolveColorNonogram(Color[,] solution, List<int>[] rowClues, List<int>[] colClues, Dictionary<RichTextBox, List<Color>> textBoxColors)
        {
            int rowCount = solution.GetLength(0);
            int colCount = solution.GetLength(1);
        ...
        {
            // Inicializáljuk a megoldó tömböt
            form.solutionBW = new int[row, col];
            for (int i = 0; i < row; i++)
                for (int j = 0; j < col; j++)
                    form.solutionBW[i, j] = 0;

            // A rekurzív megoldót hívjuk, nem a "beöntős" logikát
            // Ez a SolveRow végigpróbálja a lehetőségeket, amíg konzisztens nem lesz
            bool success = SolveRow(form.solutionBW, 0, form.rowCluesExtra, form.colCluesExtra);
            ...
            {
                MessageBox.Show("A fekete-fehér Nonogram megoldhatatlan a megadott számokkal.",
                                "Hiba", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        public bool SolveExtraNonogramColorFixed()
        {
            form.solutionColorRGB = new Color[row, col];

            // Minden cella fehér alapból
            for (int i = 0; i < row; i++)
                for (int j = 0; j < col; j++)
                    form.solutionColorRGB[i, j] = Color.White;

            // Rekurzívan oldjuk meg a nonogramot
            bool success = SolveColorRow(form.solutionColorRGB, 0, form.rowCluesExtra, form.colCluesExtra, form.textBoxColors);
            ...
            {
                MessageBox.Show("A színes Nonogram megoldhatatlan a megadott clue-okkal.", "Hiba", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return false;
            }
        }
        ...
        private bool CheckColorColumnsPartial(Color[,] solution, List<int>[] colClues, Dictionary<RichTextBox, List<Color>> textBoxColors, int maxRow)
        {
            ...
            for (int j = 0; j < colCount; j++)
            {
                ...
                for (int i = 0; i <= maxRow; i++)
                {
                    ...
                    if (cell != Color.White)
                    {
                        ...
                        else
                        {
                            ...
                            if (clueIndex >= clues.Count)
                            {
                                Console.WriteLine($"Column {j}: túl sok blokk az index {i}-nél");
                                return false;
                            }

                            if (count > clues[clueIndex])
                            {
                                Console.WriteLine($"Column {j}, clueIndex {clueIndex}: blokk túl hosszú ({count} > {clues[clueIndex]})");
                                return false;
                            }
                            ...
                            if (clueIndex < colors.Count && colors[clueIndex] != Color.Empty)
                            {
                                ...
                                {
                                    Console.WriteLine($"Column {j}, clueIndex {clueIndex}: szín eltérés (clue={colors[clueIndex]}, actual={currentColor})");
                                    return false;
                                }
                            }
                    ...
                    else
                    {
                        if (count > 0)
                        {
                            if (clueIndex >= clues.Count)
                            {
                                Console.WriteLine($"Column {j}: túl sok blokk fehér után");
                                return false;
                            }

                            if (count > clues[clueIndex])
                            {
                                Console.WriteLine($"Column {j}, clueIndex {clueIndex}: blokk túl hosszú fehér után ({count} > {clues[clueIndex]})");
                                return false;
                            }
                ...
                if (count > 0)
                {
                    if (clueIndex >= clues.Count)
                    {
                        Console.WriteLine($"Column {j}: túl sok blokk a sor végén");
                        return false;
                    }

                    if (count > clues[clueIndex])
                    {
                        Console.WriteLine($"Column {j}, clueIndex {clueIndex}: blokk túl hosszú a sor végén ({count} > {clues[clueIndex]})");
                        return false;
                    }
