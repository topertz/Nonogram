    ...
    public class NonogramRender
    {
        ...
        public void UpdatePreview(int? row = null, int? col = null)
        {
            ...
            using (Graphics graphics = Graphics.FromImage(bmp))
            {
                ...
                if (!row.HasValue) graphics.Clear(Color.White);
        ...
        public void ShowHint()
        {
            ...
            for (int rowIndex = 0; rowIndex < grid.row; rowIndex++)
            {
                for (int colIndex = 0; colIndex < grid.col; colIndex++)
                {
                    if (!grid.gridButtons[rowIndex, colIndex].Enabled) continue;
                    ...
                    if (isColorInSolution)
                    {
                        ...
                        if (!isWrong && grid.isColor && !AreColorsSimilar(grid.userColorRGB[rowIndex, colIndex], grid.solutionColorRGB[rowIndex, colIndex], 40))
                            isWrong = true;
                    }
                    else
                    {
                ...
                for (int colIndex = 0; colIndex < grid.col; colIndex++)
                {
                    ...
                    if (shouldBeColor)
                    {
                        ...
                        bool colorMatch = grid.isColor
                            ? AreColorsSimilar(grid.userColorRGB[rowIndex, colIndex], grid.solutionColorRGB[rowIndex, colIndex], 40)
            ...
            for (int rowIndex = 0; rowIndex < grid.row; rowIndex++)
            {
                for (int colIndex = 0; colIndex < grid.col; colIndex++)
                {
                    ...
                    if (btn.BackColor == Color.DarkRed)
                    {
                        ...
                        {
                            btn.BackColor = (grid.userColorRGB[rowIndex, colIndex].ToArgb() == Color.Black.ToArgb()) ? Color.Black : Color.White;
                        }
                    }
        ...
        public void ApplyColorsToBlocks()
        {
            if (!grid.isColor)
            {
                for (int rowIndex = 0; rowIndex < grid.row; rowIndex++)
                    for (int colIndex = 0; colIndex < grid.col; colIndex++)
                        grid.solutionColorRGB[rowIndex, colIndex] = grid.solutionBW[rowIndex, colIndex] == 1 ? Color.Black : Color.White;
                return;
            }
            ...
            if (usedColors.Count == 1 && grid.nonogramPalette.Length > 1)
            {
                ...
                for (int rowIndex = 0; rowIndex < grid.row; rowIndex++)
                {
                    for (int colIndex = 0; colIndex < grid.col; colIndex++)
                    {
                        if (grid.solutionBW[rowIndex, colIndex] == 1 && grid.solutionColorRGB[rowIndex, colIndex] == firstColor)
                        {
