...
namespace Nonogram
{
    public class NonogramRenderer
    {
        ...
        public void UpdatePreviewCell(int row, int col)
        {
            int cellSize = form.userCellSize;
            Bitmap bmp;

            // Ha már van kép a preview-ban, dolgozzunk azon
            ...
            {
                bmp = new Bitmap(form.previewSize, form.previewSize);
                using (Graphics g = Graphics.FromImage(bmp))
                {
                    g.Clear(Color.White);
                }
            ...
            using (Graphics g = Graphics.FromImage(bmp))
            {
                // Számoljuk a skálázást a preview méretre
                float scaleX = (float)form.previewSize / (form.col * cellSize);
                float scaleY = (float)form.previewSize / (form.row * cellSize);
                ...
                if (form.chkXMode.Checked && form.userXMark[row, col])
                {
                    // Csak akkor rajzoljuk az X-et, ha ténylegesen X a cella
                ...
                {
                    // Ha nincs X, rajzoljuk a cella aktuális színét
                    Color c = form.userColorRGB[row, col];
                    using (SolidBrush brush = new SolidBrush(c))
                    {
        ...
        public void SolveNonogram()
        {
            ...
            for (int i = 0; i < form.row; i++)
            {
                for (int j = 0; j < form.col; j++)
                {
                    ...
                    if (form.chkXMode.Checked && form.solutionColorRGB[i, j].ToArgb() == Color.White.ToArgb())
                    {
                        // X mód: minden fehér cella felkerül a listába
                        cellsToSolve.Add(new Point(i, j));
                    }
                    else if (form.userColorRGB[i, j].ToArgb() != form.solutionColorRGB[i, j].ToArgb())
                    {
        ...
        private void SolveTimer_Tick(object sender, EventArgs e)
        {
            ...
            if (form.chkXMode.Checked && form.solutionColorRGB[i, j].ToArgb() == Color.White.ToArgb())
            {
                // X mód és fehér cella → X kirakása
                btn.BackColor = Color.White;
                btn.Text = "X";
                btn.ForeColor = Color.Gray;
                btn.Font = new Font("Arial", form.userCellSize * 0.3f, FontStyle.Bold);
            ...
            for (int i = 0; i < form.row; i++)
            {
                for (int j = 0; j < form.col; j++)
                {
                    ...
                    if (isColorInSolution)
                    {
                        ...
                        if (!isWrong && form.isColor && !grid.AreColorsSimilar(form.userColorRGB[i, j], form.solutionColorRGB[i, j], 40))
                            isWrong = true;
                    }
                    else
                    {
                        ...
                        // VAGY ha be van kapcsolva az X-mód, de nincs ott vizuálisan az X szöveg
                        else if (form.chkXMode.Checked && !hasVisualX) isWrong = true;
                    }
                ...
                for (int j = 0; j < form.col; j++)
                {
                    ...
                    if (shouldBeColor)
                    {
                        ...
                        bool colorMatch = form.isColor
                            ? grid.AreColorsSimilar(form.userColorRGB[i, j], form.solutionColorRGB[i, j], 40)
                    ...
                    else
                    {
                        ...
                        else if (form.chkXMode.Checked && !hasVisualX)
                        {
                            // HIÁNYZÓ X: Kirakjuk pirossal
                            correct = false;
                            form.gridButtons[i, j].Text = "X";
                            form.gridButtons[i, j].ForeColor = Color.Red; // PIROS jelzi, hogy hiba
                            form.gridButtons[i, j].Font = new Font("Arial", form.userCellSize * 0.4f, FontStyle.Bold);
                        }
                    }
                }
            }
