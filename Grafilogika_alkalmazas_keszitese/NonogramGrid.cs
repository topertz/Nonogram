...
namespace Nonogram
{
    public class NonogramGrid
    {
        ...
        public void GenerateMediumNonogram(int gridLeft, int gridTop)
        {
            if (form.isColor)
            {
                form.nonogramColors = GetThreeRandomColorsMedium(); // Itt frissítjük a globális tömböt
            }
        ...
        public void GenerateHardNonogram()
        {
            if (form.isColor)
            {
                form.nonogramColors = GetFourRandomColorsHard(); // Új színek a nehéz pályához
            }
            int gridLeft = 20;
            ...
            while (!isBoardGood)
            {
                ...
                HashSet<Color> used = ApplyColorsToBlocks(); // Külön metódus a hard színekhez

                if (form.isColor && used.Count < form.nonogramColors.Length) continue;
