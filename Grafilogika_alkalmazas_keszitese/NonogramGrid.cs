...
namespace Grafilogika_alkalmazas_keszitese
{
    public class NonogramGrid
    {
        ...
        private async Task<bool> AnalyzeLine(int index, bool isRow)
        {
            ...
            // Megnézzük minden cellánál, hogy a sor és az oszlop színei között van-e közös.
            // Ha nincs, az a cella biztosan X.
            ...
            // Ha a kifestett cellák száma már stimmel a szabályokkal, a maradék üres hely X
            if (await HandleLogicCleanup(index, isRow, length, clues, clues.Sum())) return true;

            // Határok (Leftmost/Rightmost) kiszámítása a már javított, színérzékeny logikával
            int[] leftmost = GetLeftmost(index, isRow, length, clues);
            int[] rightmost = GetRightmost(index, isRow, length, clues);

            // Ha ellentmondás van a jelenlegi állásban (nem rakható ki a sor)
            if (leftmost == null || rightmost == null) return false;

            // --- 2. SZIGORÚ SZÍN-OVERLAP (Matematikai átfedés) ---
            for (int p = 0; p < length; p++)
            {
                int r = isRow ? index : p;
                int c = isRow ? p : index;
                ...
                for (int i = 0; i < clues.Length; i++)
                {
                    // A pixel benne van az i. blokk biztos tartományában (magjában)?
                ...
                if (potentialBlocks.Count > 0)
                {
                    ...
                    // Csak akkor festünk, ha az összes blokk, ami ideérhet, ugyanolyan színű
            ...
            // --- 3. REACHABILITY (Elérhetetlenség) ---
            for (int p = 0; p < length; p++)
            {
                int r = isRow ? index : p;
                int c = isRow ? p : index;
            ...
            // --- 4. SPECIÁLIS LOGIKÁK (Összekötés, kiterjesztés) ---
            ...
            // --- 5. HOLTPONT FELOLDÁS (Ellentmondás / Shakedown) ---
            for (int p = 0; p < length; p++)
            {
                int r = isRow ? index : p;
                int c = isRow ? p : index;
                ...
                // Próbáljuk meg: mi történne, ha ide X-et tennénk?
                userXMark[r, c] = true;
                int[] testLeft = GetLeftmost(index, isRow, length, clues);
                userXMark[r, c] = false;

                if (testLeft == null) // Ha X-szel lehetetlen kirakni a sort, akkor ide szín KELL
                {
                    ...
                    // Ha csak egyféle színű blokk érhet ide, akkor fixen az a szín lesz
                    if (possibleColors.Count == 1)
                    {
                        if (await SetCell(r, c, Color.FromArgb(possibleColors.First()), "Ellentmondás alapú kitöltés")) return true;
                    }
                }
            }
        ...
        private int[] GetLeftmost(int index, bool isRow, int length, int[] clues)
        {
            ...
            for (int i = 0; i < clues.Length; i++)
            {
                ...
                while (currentPos + clues[i] <= length)
                {
                    ...
                    {
                        // Mennyi a minimum hely az előző blokk végétől?
                        int minGap = 0;
                        if (i > 0)
                        {
                        ...
                        // Ellenőrizzük, nem hagytunk-e ki kifestett pixelt
                        bool skippedRequired = false;
                        for (int p = (i == 0 ? 0 : leftmost[i - 1] + clues[i - 1]); p < currentPos; p++)
                        {
                            if (IsFilled(isRow ? index : p, isRow ? p : index)) { skippedRequired = true; break; }
                        }

                        if (!skippedRequired && currentPos >= prevEnd)
                        {
                            leftmost[i] = currentPos;
                            // A következő keresési pontot csak a színszabály szerint toljuk el
                            currentPos += clues[i];
                            found = true;
                            break;
                        }
            ...
            for (int i = clues.Length - 1; i >= 0; i--)
            {
                ...
                while (currentPos - clues[i] >= 0)
                {
                    ...
                    if (CanPlaceBlock(index, isRow, testStart, clues[i], blockColor))
                    {
                        ...
                        bool skippedRequired = false;
                        for (int p = testStart + clues[i]; p < (i == clues.Length - 1 ? length : rightmost[i + 1]); p++)
                        {
                            if (IsFilled(isRow ? index : p, isRow ? p : index)) { skippedRequired = true; break; }
                        }

                        if (!skippedRequired && testStart + clues[i] <= nextLimit)
                        {
        ...
        private async Task<bool> SetCell(int r, int c, Color colorToSet, string reason)
        {
            ...
            MessageBox.Show($"Lépés: [{r + 1}, {c + 1}] kifestése.\n\nIndok: {reason}",
        ...
        private async Task<bool> SetX(int r, int c, string reason)
        {
            ...
            MessageBox.Show($"Lépés: [{r + 1}, {c + 1}] helyre X kerül.\n\nIndok: {reason}",
                            "AI Logika", MessageBoxButtons.OK, MessageBoxIcon.Information);
