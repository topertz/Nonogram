...
namespace Grafilogika_alkalmazas_keszitese
{
    public partial class NonogramSmartAI
    {
        ...
        public async void BtnSmartAIGuide_Click(object sender, EventArgs e)
        {
            ...
            if (solved)
                MessageBox.Show("AI megoldotta a Nonogramot! Indítsd újra a játékot az Új feladvány generálása gombbal!", "AI kész", MessageBoxButtons.OK, MessageBoxIcon.Information);
            else
        ...
        public async Task<bool> SolveWithSpeculation()
        {
            ...
            {
                changed = false;
                for (int rowIndex = 0; rowIndex < grid.row; rowIndex++) if (await AnalyzeLine(rowIndex, true)) { changed = true; break; }
                if (changed) continue;
                for (int colIndex = 0; colIndex < grid.col; colIndex++) if (await AnalyzeLine(colIndex, false)) { changed = true; break; }
            } while (changed);
            ...
            for (int rowIndex = 0; rowIndex < grid.row; rowIndex++)
            {
                for (int colIndex = 0; colIndex < grid.col; colIndex++)
                {
                    ...
                    if (!IsFilled(rowIndex, colIndex) && !IsX(rowIndex, colIndex))
                    {
                        ...
                        {
                            if (await SetX(rowIndex, colIndex, "A sor és oszlop szabályai kizárják egymást (nincs közös szín)"))
                                return await SolveWithSpeculation();
                            return false;
                        }

                        // Csak a érvényes színekhez tartozó blokkokat kérjük le
                        List<(bool isRow, int lineIdx, int blockIdx)> candidates = GetCandidateBlocks(rowIndex, colIndex).ToList();
                        ...
                        foreach ((bool isRow, int lineIdx, int blockIdx) in candidates)
                        {
                            ...
                            for (int start = leftmost[blockIdx]; start <= rightmost[blockIdx]; start++)
                            {
                                ...
                                for (int pos = 0; pos < blockLen; pos++)
                                {
                                    ...
                                    await SetCell(currentRow, currentCol, blockColor, "Spekulatív elhelyezés (backtracking)");
                                }
                        ...
                        if (await SetX(rowIndex, colIndex, "Egyik lehetséges szín sem működött ezen a ponton"))
                        {
                            if (await SolveWithSpeculation()) return true;
                        }
        ...
        {
            for (int rowIndex = 0; rowIndex < grid.row; rowIndex++)
                if (!IsLineValid(rowIndex, true)) return false;

            for (int colIndex = 0; colIndex < grid.col; colIndex++)
                if (!IsLineValid(colIndex, false)) return false;
            ...
            while (pos < length)
            {
                ...
                if (IsFilled(rowIndex, colIndex))
                {
                    ...
                    while (pos < length)
                    {
                        ...
                        if (IsFilled(currentRow, currentCol) && grid.userColorRGB[currentRow, currentCol].ToArgb() == currentColor.ToArgb())
                            pos++;
                        else
                            break;
                    }
        ...
        private async Task<bool> AnalyzeLine(int index, bool isRow)
        {
            ...
            for (int pos = 0; pos < length; pos++)
            {
                ...
                if (IsX(rowIndex, colIndex) || IsFilled(rowIndex, colIndex)) continue;

                // Biztonságosabb színlekérés a metszethez
                HashSet<int> rowColorSet = grid.rowClues[rowIndex].Select((_, idx) => GetClueColor(rowIndex, true, idx).ToArgb()).ToHashSet();
                HashSet<int> colColorSet = grid.colClues[colIndex].Select((_, idx) => GetClueColor(colIndex, false, idx).ToArgb()).ToHashSet();

                rowColorSet.IntersectWith(colColorSet);
                if (rowColorSet.Count == 0)
                {
                    if (await SetX(rowIndex, colIndex, "Szín-összeférhetetlenség (sor vagy oszlop metszet üres)")) return true;
                }
            }
            ...
            for (int pos = 0; pos < length; pos++)
            {
                ...
                if (potentialBlocks.Count > 0)
                {
                    ...
                    if (allSame)
                    {
                        if (await SetCell(rowIndex, colIndex, firstColor, "Overlap (Átfedés)")) return true;
                    }
                }
            }
            ...
            for (int pos = 0; pos < length; pos++)
            {
                ...
                if (!canAnyBlockReach)
                {
                    if (await SetX(rowIndex, colIndex, "Egyik blokk sem érhet el ide")) return true;
                }
            }
            ...
            for (int pos = 0; pos < length; pos++)
            {
                int rowIndex = isRow ? index : pos;
                int colIndex = isRow ? pos : index;
                if (IsFilled(rowIndex, colIndex) || IsX(rowIndex, colIndex)) continue;
                ...
                if (testLeft == null)
                {
                    ...
                    if (!grid.isColor)
                    {
                        ...
                        if (!IsFilled(rowIndex, colIndex))
                        {
                            if (await SetCellSafe(rowIndex, colIndex, Color.Black)) return true;
                        }
                    }
                    else
                    {
                        if (possibleColors.Count == 1)
                        {
                            ...
                            if (await SetCell(rowIndex, colIndex, targetColor, "Ellentmondás alapú kitöltés")) return true;
                        }
                    }
                }
            }
        ...
        {
            if (IsFilled(rowIndex, colIndex)) return false;
            ...
            for (int clueIndex = 0; clueIndex < clues.Length; clueIndex++)
            {
                ...
                for (int pos = rangeStart; pos <= rangeEnd; pos++)
                {
                    ...
                    if (IsFilled(rowIndex, colIndex) && grid.userColorRGB[rowIndex, colIndex].ToArgb() == clueColor.ToArgb())
                    {
                        ...
                        {
                            if (otherIdx == clueIndex) continue;
                            if (pos >= leftmost[otherIdx] && pos <= rightmost[otherIdx] + clues[otherIdx] - 1) { belongsToThis = false; break; }
                        }
                ...
                if (firstFoundPos != -1 && lastFoundPos != -1 && lastFoundPos - firstFoundPos > 0)
                {
                    for (int fillPos = firstFoundPos + 1; fillPos < lastFoundPos; fillPos++)
                    {
                        ...
                        {
                            if (await SetCell(targetRow, targetCol, clueColor, "Összekötés")) return true;
                        }
                    }
                }
            }
                    ...
                    if (CanPlaceBlock(index, isRow, currentPos, clues[clueIndex], blockColor))
                    {
                        ...
                        if (currentPos >= prevEnd)
                        {
                            ...
                            for (int pos = searchStart; pos < currentPos; pos++)
                            {
                                int rowIndex = isRow ? index : pos;
                                int colIndex = isRow ? pos : index;
                                if (IsFilled(rowIndex, colIndex)) { skippedRequired = true; break; }
                            }
                            ...
                            if (!skippedRequired)
                            {
                                ...
                                if (clueIndex < clues.Length - 1 && GetClueColor(index, isRow, clueIndex + 1).ToArgb() == blockColor.ToArgb())
                                    currentPos++;
                    ...
                    if (CanPlaceBlock(index, isRow, testStart, clues[clueIndex], blockColor))
                    {
                        ...
                        if (testStart + clues[clueIndex] <= nextLimit)
                        {
                            ...
                            if (!skippedRequired)
                            {
                                ...
                                if (clueIndex > 0 && GetClueColor(index, isRow, clueIndex - 1).ToArgb() == blockColor.ToArgb())
                                    currentPos--;
                    ...
                    if (count == 1)
                    {
                        ...
                        for (int fillPos = overlapStart; fillPos <= overlapEnd; fillPos++)
                        {
                            ...
                            if (!IsFilled(targetRow, targetCol) && !IsX(targetRow, targetCol))
                            {
                                ...
                                {
                                    if (GetClueColor(isRow ? -1 : targetRow, !isRow, crossClueIdx).ToArgb() == pixelColor.ToArgb())
                                    {
                                ...
                                {
                                    if (await SetCell(targetRow, targetCol, pixelColor, $"Horgony ({i}. blokk) kiterjesztése")) return true;
                                }
                            }
                        }
                    }
        ...
        private bool CanPlaceBlock(int lineIdx, bool isRow, int start, int len, Color color)
        {
            ...
            for (int pos = start; pos < start + len; pos++)
            {
                int rowIndex = isRow ? lineIdx : pos;
                int colIndex = isRow ? pos : lineIdx;
                if (IsX(rowIndex, colIndex)) return false;
                if (IsFilled(rowIndex, colIndex) && grid.userColorRGB[rowIndex, colIndex].ToArgb() != targetArgb) return false;
            }
            ...
            if (start > 0)
            {
                ...
                if (IsFilled(prevRow, prevCol) && grid.userColorRGB[prevRow, prevCol].ToArgb() == targetArgb) return false;
            }
            // Utána
            if (start + len < limit)
            {
                ...
                if (IsFilled(nextRow, nextCol) && grid.userColorRGB[nextRow, nextCol].ToArgb() == targetArgb) return false;
            }
            ...
            for (int pos = 0; pos < length; pos++)
            {
                ...
                if (IsFilled(rowIndex, colIndex))
                {
                    ...
                    while (pos + 1 < length && IsFilled(isRow ? index : pos + 1, isRow ? pos + 1 : index) &&
                           grid.userColorRGB[isRow ? index : pos + 1, isRow ? pos + 1 : index].ToArgb() == clusterColor.ToArgb())
                    {
                        pos++;
                    }
                    int end = pos;
                    ...
                    if (clueIdx != -1 && clues[clueIdx] == currentLen)
                    {
                        ...
                        if (start > 0)
                        {
                            ...
                            if (!IsX(prevRow, prevCol) && !IsFilled(prevRow, prevCol))
                            {
                                ...
                                if (mustHaveX)
                                {
                                    if (await SetX(prevRow, prevCol, "Kész blokk kényszerített lezárása (azonos szín miatt)")) return true;
                                }
                            }
                        }
                        ...
                        if (end < length - 1)
                        {
                            ...
                            if (!IsX(nextRow, nextCol) && !IsFilled(nextRow, nextCol))
                            {
                                ...
                                if (mustHaveX)
                                {
                                    if (await SetX(nextRow, nextCol, "Kész blokk kényszerített lezárása (azonos szín miatt)")) return true;
                                }
                            }
                        }
                    }
        ...
        private async Task<bool> HandleLogicCleanup(int index, bool isRow, int length, int[] clues, int totalBlocks)
        {
            ...
            for (int pos = 0; pos < length; pos++)
            {
                int rowIndex = isRow ? index : pos;
                int colIndex = isRow ? pos : index;
                if (IsFilled(rowIndex, colIndex)) currentFilled++;
            }
            ...
            if (currentFilled == totalBlocks)
            {
                for (int pos = 0; pos < length; pos++)
                {
                    ...
                    if (!IsFilled(rowIndex, colIndex) && !IsX(rowIndex, colIndex))
                    {
                        ...
                        if (await SetX(rowIndex, colIndex, "Sor kész (Darabszám stimmel)")) return true;
                    }
                }
            }
            return false;
        ...
        private bool IsFilled(int rowIndex, int colIndex) => grid.isColor ? grid.userColorRGB[rowIndex, colIndex] != Color.White : grid.gridButtons[rowIndex, colIndex].BackColor == Color.Black;
        private bool IsX(int rowIndex, int colIndex) => grid.userXMark[rowIndex, colIndex];
        ...
        private async Task<bool> SetCell(int rowIndex, int colIndex, Color colorToSet, string reason)
        {
            ...
            if (IsX(rowIndex, colIndex)) return false;
            if (IsFilled(rowIndex, colIndex) && grid.userColorRGB[rowIndex, colIndex].ToArgb() == colorToSet.ToArgb()) return false;
            ...
            ShowFixedDialog($"Lépés: [{rowIndex + 1}. sor, {colIndex + 1} oszlop] kifestése.\n\nIndok: {reason}");
        ...
        {
            if (IsFilled(rowIndex, colIndex) || IsX(rowIndex, colIndex)) return false;

            grid.gridButtons[rowIndex, colIndex].FlatAppearance.BorderColor = Color.Red;
            grid.gridButtons[rowIndex, colIndex].FlatAppearance.BorderSize = 3;

            ShowFixedDialog($"Lépés: [{rowIndex + 1}. sor, {colIndex + 1} oszlop] helyre X kerül.\n\nIndok: {reason}");
