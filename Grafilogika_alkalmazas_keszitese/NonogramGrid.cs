...
namespace Nonogram
{
    public class NonogramGrid
    {
        ...
        private bool IsUniqueSolution(int[,] solution)
        {
            int rowCount = solution.GetLength(0);
            int colCount = solution.GetLength(1);
            int solutionsFound = 0;

            bool Solve(int r, int c, int[,] board)
            {
                if (solutionsFound > 1) return true; // Már több megoldás van
        ...
        private bool IsUniqueColorSolution(int[,] solutionBW, Color[] colors)
        {
            int rows = solutionBW.GetLength(0);
            int cols = solutionBW.GetLength(1);

            // Soronkénti blokk-lista
            List<List<int>> rowBlocks = new List<List<int>>();
            for (int r = 0; r < rows; r++)
            {
                ...
                {
                    if (solutionBW[r, c] == 1) len++;
                    else { if (len > 0) { blocks.Add(len); len = 0; } }
                }
                if (len > 0) blocks.Add(len);
                rowBlocks.Add(blocks);
            }

            Color[,] tempBoard = new Color[rows, cols];
            int solutionsFound = 0;

            TryColorRow(0, Color.Empty, tempBoard, solutionBW, colors, rowBlocks, ref solutionsFound);

            return solutionsFound == 1;
        }

        private bool TryColorRow(int r, Color lastColor, Color[,] board, int[,] solutionBW, Color[] colors, List<List<int>> rowBlocks, ref int solutionsFound)
        {
            int rows = solutionBW.GetLength(0);

            if (solutionsFound > 1) return true; // Már több megoldás
            if (r == rows) { solutionsFound++; return false; }

            int c = 0;
            foreach (var blockLen in rowBlocks[r])
            {
                foreach (var color in colors)
                {
                    if (color == lastColor) continue; // ne egyezzen az előző blokk színével

                    // blokk ideiglenes beállítása
                    for (int k = 0; k < blockLen; k++) board[r, c + k] = color;

                    if (TryColorRow(r + 1, color, board, solutionBW, colors, rowBlocks, ref solutionsFound))
                        return true;
                }

                c += blockLen + 1; // fehér elválasztás
            }
        ...
        public void GenerateEasyNonogram(int gridLeft, int gridTop)
        {
            ...
            while (!isBoardGood)
            {
                ...
                if (form.isColor && used.Count < form.nonogramColors.Length)
                {
                    continue; // Ha nem használtunk minden színt, új pálya kell
                }
                if (!IsUniqueSolution(finalBW))
                {
        ...
        public void GenerateMediumNonogram(int gridLeft, int gridTop)
        {
            ...
            while (!isBoardGood)
            {
                ...
                while (currentPixels < targetPixels && attempts < 1500)
                {
                    ...
                    int r = rnd.Next(h);
                    int c = rnd.Next(w);
                    int len = rnd.Next(1, 5); // 1–4 hossz (jobb mediumhoz)
                ...
                if (currentPixels < minPixels) continue;
                ...
                // Színezés ellenőrzés
                HashSet<Color> usedColors = ApplyColorsToBlocks();
                if (form.isColor && usedColors.Count < form.nonogramColors.Length)
                    continue;
                if (!IsUniqueSolution(finalBW))
                {
                    continue; // Nem egyértelmű, új generálás
        ...
        public void GenerateHardNonogram(int gridLeft, int gridTop)
        {
            ...
            while (!isBoardGood)
            {
                ...
                while (currentPixels < targetPixels && attempts < 2500)
                {
                    ...
                    int r = rnd.Next(h);
                    int c = rnd.Next(w);
                    int len = rnd.Next(1, 4); // 1–3 hossz = nehezebb
