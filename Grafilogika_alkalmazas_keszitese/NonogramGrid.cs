...
namespace Grafilogika_alkalmazas_keszitese
{
    public class NonogramGrid
    {
        ...
        public NonogramGrid(Nonogram form, GameTimerManager g, NonogramRender r)
        {
            this.form = form;
            this.render = r;
        ...
        private Color[] GetTwoRandomColors()
        {
            Random rnd = form.rnd;

            // Lekérjük az összes rendszer-színt, ami nem átlátszó és nem rendszer-specifikus
            ...
            // Szűrjük a túl világos színeket (luminance)
            var darkEnoughColors = allColors
                .Where(c => (c.R * 0.2126 + c.G * 0.7152 + c.B * 0.0722) < 180)
                .ToList();

            // Véletlenszerű sorrend
            darkEnoughColors = darkEnoughColors.OrderBy(x => rnd.Next()).ToList();

            List<Color> selectedColors = new List<Color>();

            foreach (var color in darkEnoughColors)
            {
                // Ellenőrizzük, hogy a már kiválasztott színekkel hasonló-e
                if (!selectedColors.Any(c => render.AreColorsSimilar(c, color, 100))) // threshold: 100, állítható
                {
        ...
        public void GenerateEasyNonogram(int gridLeft, int gridTop)
        {
            if (form.isColor)
            {
                form.nonogramColors = GetTwoRandomColors(); // Itt frissítjük a globális tömböt
            }
            int w = 5, h = 5;
            int targetPixels = form.isColor ? 11 : 13;
            Random rnd = form.rnd;
            ...
            while (!isBoardGood)
            {
                ...
                while (currentPixels < targetPixels && attempts < 1000)
                {
                    ...
                    int len = form.isColor ? rnd.Next(1, 3) : rnd.Next(2, 4);
                    if (c + len <= w)
                    {
                        bool spaceFree = true;
                ...
                // Sűrítés
                List<int> validRows = Enumerable.Range(0, h).Where(r => Enumerable.Range(0, w).Any(c => finalBW[r, c] == 1)).ToList();
                List<int> validCols = Enumerable.Range(0, w).Where(c => Enumerable.Range(0, h).Any(r => finalBW[r, c] == 1)).ToList();
        ...
        private Color[] GetThreeRandomColors()
        {
            Random rnd = form.rnd;
            ...
            List<Color> darkEnoughColors = allColors.Where(c => (c.R * 0.2126 + c.G * 0.7152 + c.B * 0.0722) < 180).ToList();
        ...
        public void GenerateMediumNonogram(int gridLeft, int gridTop)
        {
            if (form.isColor)
            {
                form.nonogramColors = GetTwoRandomColors();
            }

            int w = 10, h = 10;
            int targetPixels = form.isColor ? 45 : 50;
            Random rnd = form.rnd;
            ...
            while (!isBoardGood)
            {
                ...
                while (currentPixels < targetPixels && attempts < 1500)
                {
                    ...
                    int r = rnd.Next(h);
                    int c = rnd.Next(w);
                    int len = form.isColor ? rnd.Next(3, 7) : rnd.Next(2, 6);
        ...
        private Color[] GetFourRandomColors()
        {
            Random rnd = form.rnd;
        ...
        public void GenerateHardNonogram(int gridLeft, int gridTop)
        {
            ...
            int w = 15, h = 15;
            int targetPixels = form.isColor ? 102 : 113;
            Random rnd = form.rnd;
        ...
        {
            if (render == null ||
                form.solutionColorRGB == null ||
                form.gridButtons == null ||
                row < 0 || row >= form.row || col < 0 || col >= form.col)
                return false;
            ...
            if (shouldBeColor)
            {
                ...
                if (form.isColor)
                    return render.AreColorsSimilar(userC, form.solutionColorRGB[row, col], 40);
        ...
        {
            if (render == null || form.solutionColorRGB == null || form.gridButtons == null)
                return false;
            for (int i = 0; i < form.row; i++)
            {
                for (int j = 0; j < form.col; j++)
        ...
        public void CreateGridUI(int gridLeft, int gridTop)
        {
            ...
            for (int i = 0; i < form.row; i++)
            {
                for (int j = 0; j < form.col; j++)
                {
                    ...
                    btn.MouseEnter += form.GridCell_MouseEnter;
                    btn.MouseLeave += form.GridCell_MouseLeave;
                    btn.Paint += (s, e) =>
                    {
                        ...
                        if (form.hintCells.Contains(p))
                        {
                            using (Brush br = new SolidBrush(Color.FromArgb(80, Color.Yellow)))
                                e.Graphics.FillRectangle(br, b.ClientRectangle);
                        }
