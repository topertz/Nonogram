            ...
            for (int rowIndex = 0; rowIndex < row; rowIndex++)
            {
                for (int colIndex = 0; colIndex < col; colIndex++)
                {
                    ...
                    if (isErrorCell[rowIndex, colIndex])
                    {
                        ...
                        // X jel visszaállítása hibás mezőnél is
                        ...
                        continue; // ne írjuk felül a színt
                    }
                    ...
                    if (!isColor && !isErrorCell[rowIndex, colIndex]) // BW mód
                    {
        ...
        public void CreateGridUI(int gridLeft, int gridTop)
        {
            ...
            if (isColor)
            {
                ...
                if (isColor && form.colorPalette.Controls.Count > 0)
                {
                    if (isZooming)
                    {
                        // Zoomkor a kiválasztott szín legyen kijelölve
                    ...
                    {
                        // Kezdetben az első szín legyen kijelölve
