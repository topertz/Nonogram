    ...
    public class ExtraGridManager
    {
        ...
        public void BtnSaveClues_Click(object sender, EventArgs e)
        {
            ...
            using (SaveFileDialog sfd = new SaveFileDialog())
            {
                ...
                MessageBox.Show(
                    "A számok sikeresen el lettek mentve!",
                    "Mentés kész",
                    MessageBoxButtons.OK,
        ...
        public void BtnLoadClues_Click(object sender, EventArgs e)
        {
            using (OpenFileDialog ofd = new OpenFileDialog())
            {
                ...
                MessageBox.Show(
                    "A számok sikeresen be lettek töltve!",
                    "Betöltés kész",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Information
                );
        ...
        public void BtnShowExtraSolution_Click(object sender, EventArgs e)
        {
            ...
            if (!grid.isColor)
            {
                ...
                if (!solvedBW)
                {
                    MessageBox.Show("A Nonogram nem oldható meg a megadott clues alapján!", "Hiba",
                ...
                for (int rowIndex = 0; rowIndex < row; rowIndex++)
                {
                    for (int colIndex = 0; colIndex < col; colIndex++)
                    {
                        ...
                        extraColors[rowIndex, colIndex] = Color.White; // previewhoz is
                    }
                }
        ...
        public void StartExtraSolveAnimation()
        {
            ...
            for (int rowIndex = 0; rowIndex < row; rowIndex++)
            {
                for (int colIndex = 0; colIndex < col; colIndex++)
                {
                    cells.Add(new Point(rowIndex, colIndex));
                }
            }

            extraSolutionQueue = new Queue<Point>(cells);
        ...
        private void ExtraSolveTimer_Tick(object sender, EventArgs e)
        {
            if (extraSolutionQueue.Count == 0)
            {
                ...
                // Animáció vége – gombok visszaengedése
                form.chkShowX.Enabled = true;
                ...
                MessageBox.Show("A nonogram teljesen kirakva!", "Megoldás kész");
                return;
            }
        ...
        public void BtnClearGrid_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("A rács és a mezők üresek, nincs mit törölni!", "Hiba", MessageBoxButtons.OK,
                    MessageBoxIcon.Warning);
                return;
            }
        ...
        private void SetMode()
        {
            ...
            if (form.rbNumberEntryMode.Checked)
            {
                ...
                form.Size = new Size(1200, 820);

                InitializeExtraRowInputs();
                InitializeExtraColumnInputs();
                InitializeExtraGrid();
            }
            // Képbeolvasós mód (blackWhite vagy color)
        ...
        private void RowClueInput_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            // Vezérlő karakterek (pl. Backspace) mindig engedélyezettek
            if (char.IsControl(e.KeyChar))
            {
                return;
            }

            // Szóköz engedélyezett
            if (e.KeyChar == ' ')
            {
                return;
            }

            // Csak számjegyek engedélyezettek
            ...
            // Ellenőrizzük, hogy a felhasználó ne kezdje egyedüli 0-val a számot
            int cursorPos = rtb.SelectionStart;
            string text = rtb.Text;

            // Ha a beszúrt számjegy 0 és:
            // vagy a mező üres, vagy
            // előtte szóköz van (új szám kezdete)
            if (e.KeyChar == '0' && (cursorPos == 0 || text[cursorPos - 1] == ' '))
            {
                e.Handled = true; // tiltás
            }

            // Ellenőrizzük, hogy az új szám ne legyen 15-nél nagyobb
            // Megkeressük az aktuális számot (a szóköz előtti karaktereket)
            int start = text.LastIndexOf(' ', cursorPos > 0 ? cursorPos - 1 : 0) + 1;
            int end = cursorPos;
            string currentNumber = text.Substring(start, end - start) + e.KeyChar;

            if (int.TryParse(currentNumber, out int value))
            {
                if (value > 15)
                {
                    e.Handled = true; // 15-nél nagyobb számot nem engedünk
        ...
        public void ApplySolutionToGridBW()
        {
            for (int rowIndex = 0; rowIndex < row; rowIndex++)
            {
                for (int colIndex = 0; colIndex < col; colIndex++)
                {
                    ...
                    {
                        extraButtons[rowIndex, colIndex].BackColor = Color.Black;
                        extraButtons[rowIndex, colIndex].Text = "";
                    }
                    else
                    {
        ...
        public void ApplySolutionToGridColor()
        {
            for (int rowIndex = 0; rowIndex < row; rowIndex++)
            {
                for (int colIndex = 0; colIndex < col; colIndex++)
                {
                    ...
                    Color cellColor = grid.solutionColorRGB[rowIndex, colIndex];

                    if (cellColor != Color.White)
                    {
                        extraButtons[rowIndex, colIndex].BackColor = cellColor;
                        extraButtons[rowIndex, colIndex].Text = "";
                    }
                    else
                    {
        ...
        // Rekurzív sor és oszlop ellenőrzés színes Nonogramhoz
        ...
        // Ellenőrzi, hogy az oszlopok megfelelnek-e teljesen a cluenak
            ...
            List<Color>> textBoxColors, int maxRow)
        {
            ...
            for (int colIndex = 0; colIndex < colCount; colIndex++)
            {
                ...
                for (int rowIndex = 0; rowIndex <= maxRow; rowIndex++)
                {
                    ...
                    if (cell != Color.White)
                    {
                        ...
                        else
                        {
                            ...
                            // Ellenőrizzük a színt csak akkor, ha a clue színe nem üres
                ...
                // Ellenőrizzük a maradék blokkot a sor végén
                if (count > 0)
                {
                    if (clueIndex >= clues.Count)
                    {
        ...
        {
            // Grid tartalmának törlése (csak a cellák)
            if (extraButtons != null)
            {
                for (int rowIndex = 0; rowIndex < extraButtons.GetLength(0); rowIndex++)
                {
