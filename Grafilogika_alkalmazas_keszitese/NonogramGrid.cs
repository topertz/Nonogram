...
namespace Grafilogika_alkalmazas_keszitese
{
    public class NonogramGrid
    {
        ...
        public void GridCell_MouseEnter(object sender, EventArgs e)
        {
            ...
            Point point = (Point)btn.Tag;
            highlightedRow = point.X;
            highlightedCol = point.Y;
            ...
            while (!isBoardGood && attemptOverall < maxAttempts)
            {
                ...
                while (currentPixels < targetPixels && attempts < maxAttempts)
                {
                    ...
                    for (int offset = 0; offset < len; offset++)
                        if (finalBW[row, startCol + offset] == 1) { free = false; break; }
                    if (!free) continue;
                    ...
                    {
                        // Nézzük a bal oldalt
                        if (startCol > 0 && finalBW[row, startCol - 1] == 1) len = 2;
                        // Nézzük a jobb oldalt
                        else if (startCol + 1 < width && finalBW[row, startCol + 1] == 1) { startCol = startCol - 1; len = 2; }
                        else continue; // ne engedjük létrejönni
                    }
                ...
                for (int rowIndex = 0; rowIndex < height && !error; rowIndex++)
                    if (!Enumerable.Range(0, width).Any(colIndex => finalBW[rowIndex, colIndex] == 1)) error = true;
                for (int colIndex = 0; colIndex < width && !error; colIndex++)
                    if (!Enumerable.Range(0, height).Any(rowIndex => finalBW[rowIndex, colIndex] == 1)) error = true;
                if (error) continue;
                ...
                for (int rowIndex = 0; rowIndex < height - 1 && !error; rowIndex++)
                    for (int nextRowIndex = rowIndex + 1; nextRowIndex < height; nextRowIndex++)
                        if (Enumerable.Range(0, width).All(colIndex => finalBW[rowIndex, colIndex] == finalBW[nextRowIndex, colIndex])) error = true;
                if (error) continue;
                ...
                if (!solver.HasLogicalStartMoves()) continue;
        ...
        {
            int max = 0;
            foreach (int[] array in clues) if (array.Length > max) max = array.Length;
            return max;
        }
                ...
                for (int colIndex = 0; colIndex < col; colIndex++)
                {
                    ...
                    btn.Paint += (s, e) =>
                    {
                        ...
                        using (Pen thickPen = new Pen(Color.Black, 2))
                        {
                            ...
                            if (targetCol == col - 1) e.Graphics.DrawLine(thickPen, btn.Width - 1, 0, btn.Width - 1, btn.Height);
                            if (targetRow == row - 1) e.Graphics.DrawLine(thickPen, 0, btn.Height - 1, btn.Width, btn.Height - 1);
                        }
                    };
                    btn.MouseDown += (s, e) =>
                    {
            ...
            if (isColor)
            {
                ...
                for (int rowIndex = 0; rowIndex < row; rowIndex++)
                    for (int colIndex = 0; colIndex < col; colIndex++)
                    {
                        ...
                        if (color != Color.White) colorsSet.Add(color.ToArgb());
                    }
        ...
        public void GenerateNonogramFromImage(Image img, int gridLeft, int gridTop)
        {
            ...
            for (int rowIndex = 0; rowIndex < row; rowIndex++)
            {
                ...
                while (colIndex < col)
                {
                    if (!fillableCells[rowIndex, colIndex]) { colIndex++; continue; }
                    ...
                    while (blockEndIndex < col && fillableCells[rowIndex, blockEndIndex])
                    {
                        ...
                        if (isColor && count > 0 && !render.AreColorsSimilar(seed, color, colorSimilarityThreshold)) break;
                        count++; blockEndIndex++;
                    }
                    clues.Add(count);
            ...
            for (int colIndex = 0; colIndex < col; colIndex++)
            {
                ...
                while (rowIndex < row)
                {
                    if (!fillableCells[rowIndex, colIndex]) { rowIndex++; continue; }
