    ...
    public partial class LeaderboardManager
    {
        ...
        public DataTable LoadAllSaves(string folderPath)
        {
            ...
            foreach (string file in Directory.GetFiles(folderPath, "*.json"))
            {
                ...
                {
                    Console.WriteLine($"Hiba a fájl deszerializálásánál: {file} -> {ex.Message}");
                }
            }
            ...
            List<NonogramSaveData> sorted = saves
                ...
                .ThenBy(s => s.WrongCellClicks + (s.Mode == "Színes" ? s.WrongColorClicks.GetValueOrDefault() : 0))
                .ToList();
        ...
        public void ShowLeaderboard(DataTable table)
        {
            ...
            dataGridView.Columns.Add(new DataGridViewTextBoxColumn
            {
                Name = "Játékmód",
                HeaderText = "Játékmód",
                DataPropertyName = "Játékmód"
            });

            dataGridView.Columns.Add(new DataGridViewTextBoxColumn
            {
                Name = "WrongCells",
                HeaderText = "Cellahiba",
                DataPropertyName = "Helytelen cella kattintások száma"
            });
            ...
            dataGridView.Columns.Add(new DataGridViewTextBoxColumn
            {
                Name = "Hints",
                HeaderText = "Segítség",
                DataPropertyName = "Segítségek száma"
            });
        ...
        private void ApplyFilter()
        {
            ...
            bool containsColorMode = bindingSource.Cast<DataRowView>().Any(r => r["Játékmód"].ToString() == "Színes");
            dgvLeaderboard.Columns["Helytelen színek száma"].Visible = (mode == "Színes"
                || (mode == "Összes" && containsColorMode));
