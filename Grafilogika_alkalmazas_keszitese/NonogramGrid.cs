        ...
        public void GenerateNonogram(int gridLeft, int gridTop, int width, int height, int targetPixels, int maxAttempts)
        {
            ...
            while (!isBoardGood)
            {
                ...
                while (currentPixels < targetPixels && attempts < maxAttempts)
                {
                    ...
                    // Biztosítjuk, hogy a blokk ne lógjon túl
                    int maxStart = width - len;
                    if (maxStart < 0) continue; // túl nagy blokk, kihagyjuk
                    int c = rnd.Next(0, maxStart + 1);

                    // Ellenőrizzük, hogy szabad-e a hely
                    bool free = true;
                    for (int k = 0; k < len; k++)
                    {
                        if (finalBW[r, c + k] == 1)
