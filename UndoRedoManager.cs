using System;
using System.Collections.Generic;
using System.Drawing;
using System.Windows.Forms;

namespace Grafilogika_alkalmazas_keszitese
{
    public class UndoRedoManager
    {
        public Stack<Tuple<Color[,], int, int, bool>> undoStack = new Stack<Tuple<Color[,], int, int, bool>>();
        public Stack<Tuple<Color[,], int, int, bool>> redoStack = new Stack<Tuple<Color[,], int, int, bool>>();
        public bool lastActionWasX = false;
        public int undoClicks = 0;
        public int maxUndoClicks = 0;
        private Nonogram form;
        private NonogramGrid grid;

        public UndoRedoManager(Nonogram f, NonogramGrid g)
        {
            this.form = f;
            this.grid = g;
        }
        public void SetGrid(NonogramGrid grid)
        {
            this.grid = grid;
        }
        public void BtnUndo_Click(object sender, EventArgs e)
        {
            Undo();
        }

        public void BtnRedo_Click(object sender, EventArgs e)
        {
            Redo();
        }
        public void SaveState(bool wasX = false)
        {
            if (grid.gridButtons == null)
            {
                return;
            }
            Color[,] clone = CloneGrid();
            undoStack.Push(Tuple.Create(clone, grid.wrongCellClicks, grid.wrongColorClicks, wasX));
            redoStack.Clear();
            bool isEasy = form.cmbDifficulty.SelectedIndex == 0;
            form.btnUndo.Enabled = undoStack.Count > 0 && (isEasy || undoClicks < maxUndoClicks);
            form.btnRedo.Enabled = redoStack.Count > 0;
        }

        public void Undo()
        {
            bool isEasy = form.cmbDifficulty.SelectedIndex == 0;
            if (undoStack.Count == 0 || (!isEasy && undoClicks >= maxUndoClicks))
            {
                MessageBox.Show("No previous status!", "Undo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                if (!isEasy)
                {
                    form.btnUndo.Enabled = false;
                }
                return;
            }

            // Save current state to redo stack
            Color[,] currentClone = CloneGrid();
            bool wasXCurrent = false;
            if (undoStack.Count > 0)
            {
                wasXCurrent = undoStack.Peek().Item4;
            }
            redoStack.Push(Tuple.Create(currentClone, grid.wrongCellClicks, grid.wrongColorClicks, wasXCurrent));

            // Restore previous state
            Tuple<Color[,], int, int, bool> state = undoStack.Pop();
            RestoreState(state.Item1);  // rács
            grid.wrongCellClicks = state.Item2;
            grid.wrongColorClicks = state.Item3;
            bool wasX = state.Item4;

            // Update labels
            form.lblWrongCellClicks.Text = $"Number of incorrect clicks: {grid.wrongCellClicks} " +
                $"(max: {grid.maxWrongCellClicks})";
            form.lblWrongColorClicks.Text = $"Number of colors: {grid.wrongColorClicks} " +
                $"(max: {grid.maxWrongColorClicks})";

            lastActionWasX = wasX;
            if (!isEasy && !wasX)
            {
                if (undoClicks < maxUndoClicks)
                {
                    undoClicks++;
                }
                else
                {
                    MessageBox.Show("You have reached the maximum withdrawal!");
                }
            }
            UpdateUndoLabel();

            form.btnUndo.Enabled = undoStack.Count > 0 && (isEasy || undoClicks < maxUndoClicks);
            form.btnRedo.Enabled = redoStack.Count > 0;
        }

        public void Redo()
        {
            bool isEasy = form.cmbDifficulty.SelectedIndex == 0;
            if (redoStack.Count == 0)
            {
                MessageBox.Show("There is no later state!", "Redo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                if (!isEasy)
                {
                    form.btnRedo.Enabled = false;
                }
                return;
            }

            // Save current state to undo stack
            Color[,] currentClone = CloneGrid();
            bool wasXCurrent = false;
            if (redoStack.Count > 0)
            {
                wasXCurrent = redoStack.Peek().Item4;
            }
            undoStack.Push(Tuple.Create(currentClone, grid.wrongCellClicks, grid.wrongColorClicks, wasXCurrent));

            // Reset next state
            Tuple<Color[,], int, int, bool> state = redoStack.Pop();
            RestoreState(state.Item1);  // rács
            grid.wrongCellClicks = state.Item2;
            grid.wrongColorClicks = state.Item3;
            bool wasX = state.Item4;

            // Update labels
            form.lblWrongCellClicks.Text = $"Number of incorrect clicks: {grid.wrongCellClicks} " +
                $"(max: {grid.maxWrongCellClicks})";
            form.lblWrongColorClicks.Text = $"Number of colors: {grid.wrongColorClicks} " +
                $"(max: {grid.maxWrongColorClicks})";

            lastActionWasX = wasX;
            UpdateUndoLabel();

            form.btnUndo.Enabled = undoStack.Count > 0 && (isEasy || undoClicks < maxUndoClicks);
            form.btnRedo.Enabled = redoStack.Count > 0;
        }

        public void UpdateUndoLabel()
        {
            form.lblUndoCount.Text = $"Number of withdrawals: {undoClicks} (max: {maxUndoClicks})";
        }

        private Color[,] CloneGrid()
        {
            Color[,] clone = new Color[grid.row, grid.col];
            Color xColorMarker = Color.FromArgb(255, 255, 254);

            for (int rowIndex = 0; rowIndex < grid.row; rowIndex++)
            {
                for (int colIndex = 0; colIndex < grid.col; colIndex++)
                {
                    // If there is an X, then save the marker color, otherwise the regular color
                    clone[rowIndex, colIndex] = grid.userXMark[rowIndex, colIndex] ? xColorMarker
                        : grid.userColorRGB[rowIndex, colIndex];
                }
            }
            return clone;
        }

        private void RestoreState(Color[,] state)
        {
            Color xColorMarker = Color.FromArgb(255, 255, 254);

            for (int rowIndex = 0; rowIndex < grid.row; rowIndex++)
            {
                for (int colIndex = 0; colIndex < grid.col; colIndex++)
                {
                    if (grid.isHintFixed[rowIndex, colIndex])
                    {
                        continue;
                    }
                    Color storedColor = state[rowIndex, colIndex];

                    if (storedColor.ToArgb() == xColorMarker.ToArgb())
                    {
                        // This was an X mark
                        grid.userXMark[rowIndex, colIndex] = true;
                        grid.userColorRGB[rowIndex, colIndex] = Color.White;
                        grid.gridButtons[rowIndex, colIndex].BackColor = Color.White;

                        // Display the X symbol visually
                        float fontSize = grid.userCellSize * 0.3f;
                        grid.gridButtons[rowIndex, colIndex].Font = new Font("Arial", fontSize, FontStyle.Bold);
                        grid.gridButtons[rowIndex, colIndex].ForeColor = Color.Gray;
                        grid.gridButtons[rowIndex, colIndex].Text = "X";
                    }
                    else
                    {
                        // This was a plain color
                        grid.userXMark[rowIndex, colIndex] = false;
                        grid.userColorRGB[rowIndex, colIndex] = storedColor;
                        grid.gridButtons[rowIndex, colIndex].BackColor = storedColor;
                        grid.gridButtons[rowIndex, colIndex].Text = "";
                    }
                }
            }
            form.render.UpdatePreview();
        }

        public void ClearHistory()
        {
            undoStack.Clear();
            redoStack.Clear();
            form.btnUndo.Enabled = undoStack.Count > 0 && (form.cmbDifficulty.SelectedIndex == 0 || undoClicks < maxUndoClicks);
            form.btnRedo.Enabled = redoStack.Count > 0;
        }
    }
}