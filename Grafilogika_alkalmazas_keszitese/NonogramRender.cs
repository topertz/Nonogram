...
namespace Grafilogika_alkalmazas_keszitese
{
    public class NonogramRender
    {
            ...
            using (Graphics g = Graphics.FromImage(bmp))
            {
                ...
                for (int i = startRow; i <= endRow; i++)
                {
                    for (int j = startCol; j <= endCol; j++)
                    {
                        ...
                        if (form.isXMode && grid.userXMark[i, j])
                        {
        ...
        public void ClearErrorHighlights()
        {
            for (int i = 0; i < grid.row; i++)
            {
                for (int j = 0; j < grid.col; j++)
                {
                    ...
                    {
                        btn.Text = "";
                        grid.userXMark[i, j] = false; // Logikailag is ürítjük
                    }
                }
            }
        }
        ...
        public void SetCellColor(int row, int col, Button btn, Color color)
        {
            ...
            grid.userColor[row, col] = 1; // jelzés, hogy kitöltve
            btn.BackColor = color;
            btn.Text = "";
        ...
        public void ChkShowX_CheckedChanged(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < grid.gridButtons.GetLength(0); i++)
            {
                for (int j = 0; j < grid.gridButtons.GetLength(1); j++)
                {
                    ...
                    if (grid.gridButtons[i, j] != null)
                    {
                        //form.gridButtons[i, j].Enabled = !show;
                        grid.gridButtons[i, j].TabStop = false;
                    }
                }
            }
        ...
        public Color[] GetTwoRandomColors()
        {
            ...
            foreach (Color color in darkEnoughColors)
            {
                ...
                if (!selectedColors.Any(c => AreColorsSimilar(c, color, 100))) // threshold: 100, állítható
                {
        ...
        public void ApplyColorsToBlocks()
        {
            ...
            for (int i = 0; i < grid.row; i++)
            {
                ...
                while (j < grid.col)
                {
                    ...
                    // Új blokk kezdete
                    List<Color> availableColors = grid.nonogramColors
                    .Where(c => c != lastColor)
                    .OrderBy(x => rnd.Next())
                    .Take(grid.isColor ? 2 : grid.nonogramColors.Length)
                    .ToList();
            ...
            if (usedColors.Count == 1 && grid.nonogramColors.Length > 1)
            {
                Color firstColor = usedColors.First();
                Color secondColor = grid.nonogramColors.First(c => c != firstColor);
