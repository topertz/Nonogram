...
namespace Nonogram
{
    public class NonogramRenderer
    {
                    ...
                    for (int j = 0; j < cols; j++)
                    {
                        ...
                        if (form.isXMode && form.userXMark[i, j])
                        {
                            ...
                            using (SolidBrush brush = new SolidBrush(Color.Gray))
                            {
                                ...
                                g.DrawString("X", font, brush, new RectangleF(j * cellSize, i * cellSize, cellSize, cellSize), sf);
                            }
                        }
                        else
                        {
        ...
        public void UpdatePreviewCell(int row, int col)
        {
            ...
            {
                // Ha még nincs kép, hozzunk létre egy üreset
                form.picPreview.Image = new Bitmap(form.previewSize, form.previewSize);
                using (Graphics g = Graphics.FromImage(form.picPreview.Image))
                    g.Clear(Color.White);
            }

            Bitmap bmp = new Bitmap(form.picPreview.Image);
            using (Graphics g = Graphics.FromImage(bmp))
            {
                int cellSize = form.userCellSize;
                ...
                // X vagy szín rajzolása
                if (form.isXMode && form.userXMark[row, col])
                {
        ...
        public void CheckSolution()
        {
            ...
            if (correct || form.chkBlackWhiteHardMode.Checked || form.chkColorHardMode.Checked)
            {
