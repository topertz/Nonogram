...
namespace Nonogram
{
    public class NonogramGrid
    {
        ...
        private bool IsUniqueSolution()
        {
            ...
            int[,] testBoard = new int[form.row, form.col];
            Color[,] testColors = new Color[form.row, form.col];

            SolveRecursive(0, 0, testBoard, testColors);

            // Ha lejárt az idő (timeout), a solutionsFound valószínűleg 0 vagy 1, 
            // de nem vagyunk biztosak benne -> adjunk vissza false-t, hogy generáljon újat.
        ...
        private void SolveRecursive(int r, int c, int[,] board, Color[,] boardColors)
        {
            ...
            if ((DateTime.Now - solverStartTime).TotalMilliseconds > 500) return; // Szigorúbb limit
            ...
            int nextR = (c + 1 == form.col) ? r + 1 : r;
            int nextC = (c + 1 == form.col) ? 0 : c + 1;

            // --- Optimalizáció: Sor végén teljes ellenőrzés ---
            bool isEndOfRow = (c == form.col - 1);
            ...
            // Színes/Fekete kipróbálása
            if (solutionsFound > 1) return;

            Color[] possibleColors = form.isColor ? form.nonogramColors : new Color[] { Color.Black };
        ...
        {
            // Ellenőrzés soron (r) - közvetlenül a board-on, nincs ExtractRow!
            if (!IsLineMatchOptimized(r, c, true, board, boardColors)) return false;

            // Ellenőrzés oszlopon (c) - közvetlenül a board-on, nincs ExtractCol!
            if (!IsLineMatchOptimized(r, c, false, board, boardColors)) return false;

            return true;
        }

        private int[] ExtractRow(int[,] board, int r, int length)
        {
            int[] row = new int[length];
            for (int c = 0; c < length; c++) row[c] = board[r, c];
            return row;
        }

        private Color[] ExtractRowColors(Color[,] boardColors, int r, int length)
        {
            Color[] colors = new Color[length];
            for (int c = 0; c < length; c++) colors[c] = boardColors[r, c];
            return colors;
        }

        private int[] ExtractCol(int[,] board, int c, int length)
        {
            int[] col = new int[length];
            for (int r = 0; r < length; r++) col[r] = board[r, c];
            return col;
        }

        private Color[] ExtractColColors(Color[,] boardColors, int c, int length)
        {
            Color[] colors = new Color[length];
            for (int r = 0; r < length; r++) colors[r] = boardColors[r, c];
            return colors;
        }

        private bool IsLineMatchOptimized(int r, int c, bool isRow, int[,] board, Color[,] boardColors)
        {
            int limit = isRow ? c + 1 : r + 1; // Meddig tart a kitöltött rész
            int lineLength = isRow ? form.col : form.row;
            bool isComplete = isRow ? (c == form.col - 1) : (r == form.row - 1);

            int[] clues = isRow ? form.rowClues[r] : form.colClues[c];
            Color[] clueColors = isRow ? form.rowClueColors[r] : form.colClueColors[c];
            ...
            for (int i = 0; i < limit; i++)
            {
                ...
                if (val == 1) // Színes cella
                {
                    if (currentBlockSize == 0) // Új blokk kezdődik
                    {
                        currentBlockColor = col;
                        currentBlockSize = 1;
                    }
                    else if (form.isColor && col != currentBlockColor) // Színváltás (csak színes módban)
                    {
                        // Lezárjuk az előzőt
                    ...
                    else // Blokk folytatódik
                    {
                        currentBlockSize++;
                    }

                    // Ha már most hosszabb a blokk, mint a clue, az hiba
                    if (currentClueIdx >= clues.Length || currentBlockSize > clues[currentClueIdx])
                        return false;
                }
                else // Fehér cella (0)
                {
                    if (currentBlockSize > 0) // Most ért véget egy blokk
                    {
            ...
            // Ha a sor/oszlop végére értünk
            if (isComplete)
            {
                if (currentBlockSize > 0) // Az utolsó blokk lezárása
                {
                    if (currentClueIdx != clues.Length - 1 || currentBlockSize != clues[currentClueIdx] || currentBlockColor != clueColors[currentClueIdx])
                        return false;
                    currentClueIdx++;
                }
                return currentClueIdx == clues.Length; // Minden clue megvan-e
            }

            return true;
        }

        private void Solve(int r, int c, int rowCount, int colCount, int[,] board, int[,] solution, ref int solutionsFound)
        {
            if (solutionsFound > 1) return; // Már több megoldás van, megállunk

            if (r == rowCount) // Végigértünk az összes soron
            {
                solutionsFound++;
                return;
            }

            int nextR = (c + 1 == colCount) ? r + 1 : r;
            int nextC = (c + 1 == colCount) ? 0 : c + 1;

            // A te logikád alapján a solution értékeit követve:
            if (solution[r, c] == 1) // fekete cella
            {
                board[r, c] = 1;
                Solve(nextR, nextC, rowCount, colCount, board, solution, ref solutionsFound);
            }
            else // fehér cella
            {
                board[r, c] = 0;
