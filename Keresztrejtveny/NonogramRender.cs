...
namespace Nonogram
{
    public class NonogramRenderer
    {
        ...
        public NonogramRenderer(Form1 f, NonogramGrid g)
        {
            form = f;
            grid = g;
        }
        ...
        public void CheckSolution()
        {
            ...
            for (int i = 0; i < form.row; i++)
            {
                for (int j = 0; j < form.col; j++)
                {
                    ...
                    if (form.isColor)
                    {
                        Color userColor = form.userColorRGB[i, j] == default ? Color.White : form.userColorRGB[i, j];
                    ...
                    if (wrong)
                    {
                        ...
                        form.gridButtons[i, j].BackColor = Color.Red;
                    }
                    else
                    {
            ...
            for (int i = 0; i < form.gridButtons.GetLength(0); i++)
            {
                for (int j = 0; j < form.gridButtons.GetLength(1); j++)
                {
                    if (form.userXMark[i, j])
                        form.gridButtons[i, j].Text = show ? "X" : "";
                }
            }
        }
