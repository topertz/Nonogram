...
namespace Grafilogika_alkalmazas_keszitese
{
    public partial class NonogramSmartAI
    {
        ...
        public async void BtnSmartAIGuide_Click(object sender, EventArgs e)
        {
            ...
            if (solved)
            {
                MessageBox.Show("AI megoldotta a Nonogramot! Indítsd újra a játékot az Új feladvány generálása gombbal!",
                    "AI kész", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                MessageBox.Show("AI nem talál több biztos lépést.", "AI kész",
                    ...
                    if (!IsFilled(rowIndex, colIndex) && !IsX(rowIndex, colIndex))
                    {
                        ...
                        if (validColors.Count == 0)
                        {
                            if (await SetX(rowIndex, colIndex,
                                "A sor és oszlop szabályai kizárják egymást (nincs közös szín)"))
                            {
        ...
        private async Task<bool> AnalyzeLine(int index, bool isRow)
        {
            ...
            for (int pos = 0; pos < length; pos++)
            {
                ...
                {
                    if (await SetX(rowIndex, colIndex, "Szín-összeférhetetlenség (sor vagy oszlop metszet üres)"))
                    {
                        return true;
                    }
                }
            ...
            for (int pos = 0; pos < length; pos++)
            {
                ...
                if (potentialBlocks.Count > 0)
                {
                    ...
                    if (allSame)
                    {
                        if (await SetCell(rowIndex, colIndex, firstColor, "Overlap (Átfedés)"))
                        {
                            return true;
                        }
                    }
            ...
            for (int pos = 0; pos < length; pos++)
            {
                ...
                if (!canAnyBlockReach)
                {
                    if (await SetX(rowIndex, colIndex, "Egyik blokk sem érhet el ide"))
                    {
                        return true;
                    }
                }
                ...
                if (testLeft == null)
                {
                    ...
                    else
                    {
                        if (possibleColors.Count == 1)
                        {
                            ...
                            if (await SetCell(rowIndex, colIndex, targetColor, "Ellentmondás alapú kitöltés"))
                            {
                                return true;
                            }
                        }
            ...
            for (int clueIndex = 0; clueIndex < clues.Length; clueIndex++)
            {
                ...
                if (firstFoundPos != -1 && lastFoundPos != -1 && lastFoundPos - firstFoundPos > 0)
                {
                    for (int fillPos = firstFoundPos + 1; fillPos < lastFoundPos; fillPos++)
                    {
                        ...
                        {
                            if (await SetCell(targetRow, targetCol, clueColor, "Összekötés"))
                            {
                                return true;
                            }
                        }
                    ...
                    if (count == 1)
                    {
                        ...
                        for (int fillPos = overlapStart; fillPos <= overlapEnd; fillPos++)
                        {
                            ...
                            if (!IsFilled(targetRow, targetCol) && !IsX(targetRow, targetCol))
                            {
                                ...
                                {
                                    if (await SetCell(targetRow, targetCol, pixelColor, $"Horgony ({i}. blokk) kiterjesztése"))
                                    {
                                        return true;
                                    }
                                }
                    ...
                    if (clueIdx != -1 && clues[clueIdx] == currentLen)
                    {
                        ...
                        if (start > 0)
                        {
                            ...
                            if (!IsX(prevRow, prevCol) && !IsFilled(prevRow, prevCol))
                            {
                                ...
                                if (mustHaveX)
                                {
                                    if (await SetX(prevRow, prevCol, "Kész blokk kényszerített lezárása (azonos szín miatt)"))
                                    {
                                        return true;
                                    }
                                }
                        ...
                        if (end < length - 1)
                        {
                            ...
                            if (!IsX(nextRow, nextCol) && !IsFilled(nextRow, nextCol))
                            {
                                ...
                                if (mustHaveX)
                                {
                                    if (await SetX(nextRow, nextCol, "Kész blokk kényszerített lezárása (azonos szín miatt)"))
                                    {
                                        return true;
                                    }
                                }
            ...
            if (currentFilled == totalBlocks)
            {
                for (int pos = 0; pos < length; pos++)
                {
                    ...
                    if (!IsFilled(rowIndex, colIndex) && !IsX(rowIndex, colIndex))
                    {
                        ...
                        if (await SetX(rowIndex, colIndex, "Sor kész (Darabszám stimmel)"))
                        {
                            return true;
                        }
                    }
