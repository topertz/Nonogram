...
namespace Grafilogika_alkalmazas_keszitese
{
    public class NonogramRender
    {
        ...
        public NonogramRender(Nonogram f, NonogramGrid g, GameTimerManager game, UndoRedoManager u, SaveLoadManager s)
        {
            this.form = f;
            this.grid = g;
            this.gameTimerManager = game;
            this.undoredoManager = u;
            this.saveLoadManager = s;
        }
        ...
        public Color[] GetTwoRandomColors()
        {
            ...
            foreach (Color color in darkEnoughColors)
            {
                ...
                if (!selectedColors.Any(c => AreColorsSimilar(c, color, 100)))
                {
