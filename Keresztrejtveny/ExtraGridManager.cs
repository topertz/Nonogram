            ...
            for (int j = 0; j < col; j++)
            {
                ...
                for (int i = 0; i <= maxRow; i++)
                {
                    ...
                    if (cell != Color.White)
                    {
                        ...
                        {
                            // blokkváltás
                            if (clueIndex < clues.Count && count > clues[clueIndex])
                                return false; // túl hosszú blokk
                            if (clueIndex < colors.Count && clueIndex < clues.Count && colors[clueIndex] != currentColor)
                                return false; // szín nem egyezik
                            clueIndex++;
                            currentColor = cell;
                            count = 1;
                        }
