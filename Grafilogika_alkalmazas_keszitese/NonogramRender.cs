...
namespace Grafilogika_alkalmazas_keszitese
{
    public class NonogramRender
    {
            ...
            using (Graphics graphics = Graphics.FromImage(bmp))
            {
                ...
                for (int rowIndex = startRow; rowIndex <= endRow; rowIndex++)
                {
                    for (int colIndex = startCol; colIndex <= endCol; colIndex++)
                    {
                        ...
                        if (grid.userXMark[rowIndex, colIndex] || grid.aiXMark[rowIndex, colIndex])
                        {
                            ...
                            using (SolidBrush brush = new SolidBrush(Color.Gray))
                            {
                                ...
                                graphics.DrawString("X", font, brush, cellRect, sf);
                            }
                        }
                        else
                        {
        ...
        public void SolveNonogram()
        {
            ...
            form.chkImgBlackWhiteMode.Enabled = false;
            form.chkImgColorMode.Enabled = false;
        ...
        private void SolveTimer_Tick(object sender, EventArgs e)
        {
            if (solutionQueue.Count == 0)
            {
                solveTimer.Stop();
                solveTimer.Dispose();
                SetGridEnabled(true, form.chkImgColorMode.Checked, form.chkImgBlackWhiteMode.Checked);
                gameTimerManager.Stop();
                if (MessageBox.Show("A nonogram teljesen kirakva!", "Megoldás kész") == DialogResult.OK)
                {
                    ...
                    form.chkImgBlackWhiteMode.Enabled = true;
                    form.chkImgColorMode.Enabled = true;
                    ...
                    if (!form.chkImgBlackWhiteMode.Checked && !form.chkImgColorMode.Checked)
                    {
        ...
        public void CheckSolution()
        {
            ...
            if (correct || form.chkImgBlackWhiteMode.Checked || form.chkImgColorMode.Checked)
            {
                form.btnCheck.Enabled = true;
            ...
            {
                MessageBox.Show("Vannak hibás mezők! Kattints a rácsra a javításhoz.", "Ellenőrzés",
        ...
        public void BtnSelectImage_Click(object sender, EventArgs e)
        {
            ...
            bool useGrayscale = form.chkImgBlackWhiteMode.Checked;
            ...
            if (form.chkImgColorMode.Checked)
            {
        ...
        public Color[] GetTwoRandomColors()
        {
            ...
            return new Color[]
            {
                grid.nonogramPalette[firstIndex], grid.nonogramPalette[secondIndex]
            };
        }
        public void ApplyColorsToBlocks()
        {
            if (!grid.isColor)
            {
                for (int rowIndex = 0; rowIndex < grid.row; rowIndex++)
                {
                    for (int colIndex = 0; colIndex < grid.col; colIndex++)
                    {
                        grid.solutionColorRGB[rowIndex, colIndex] = grid.solutionBW[rowIndex, colIndex] == 1
                            ? Color.Black : Color.White;
                    }
                }
                return;
            ...
            if (usedColors.Count == 1 && grid.nonogramPalette.Length > 1)
            {
                ...
                for (int rowIndex = 0; rowIndex < grid.row; rowIndex++)
                {
                    for (int colIndex = 0; colIndex < grid.col; colIndex++)
                    {
                        if (grid.solutionBW[rowIndex, colIndex] == 1 && grid.solutionColorRGB[rowIndex, colIndex]
                            == firstColor)
                        {
        ...
        public void FinalizeGame()
        {
            ...
            try
            {
                ...
                MessageBox.Show("A játék automatikusan elmentve!", "Mentés",
